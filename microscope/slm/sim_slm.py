...
class SIM_SLM(microscope.abc.Device):
    ...
    def __init__(
        ...
        self._pixel_pitch = (
            pixel_pitch  # microns  # TODO: This should go into the SLM
        )
        self._sim_phase_offset = 0.0
        self._sim_angle_offset = TWO_PI / 5.0
        self._sim_num_phases = 5
        self._sim_num_angles = 3

        self.kk, self.ll = np.meshgrid(
            np.arange(self._slm.get_shape()[0]),
            np.arange(self._slm.get_shape()[1]),
        )
    ...
    def set_sim_modulation_factors(self, factors: dict):
        self._sim_modulation_factors = {
            int(w): int(f) for w, f in factors.items()
        }
    ...
    def set_sim_sequence(self, angle_phase_wavelength):
        ...
        """
        logging.info(
            f"calling set_sim_sequence with: {angle_phase_wavelength}"
        )
        num_phases = 0
        num_angles = 0
        wavelengths = []
        ...
        phases = [
            self._sim_phase_offset + n * TWO_PI / num_phases
            for n in range(num_phases)
        ]
        angles = [
            self._sim_angle_offset + n * TWO_PI / num_angles
        ...
        pitches = {
            w: w
            / (1000.0 * np.sin(self._sim_diffraction_angle * TWO_PI / 360.0))
            for w in wavelengths
        }

        patterns = np.zeros(
            (len(angle_phase_wavelength), *self._slm.get_shape()),
            dtype=np.float32,
        )
        wavelength_seq = []
